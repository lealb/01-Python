...
class ZLSpider(object):
    ...
    def __init__(self):
        ...
        # 创建数据库和相关表
        self.cur.execute('CREATE DATABASE IF NOT EXISTS Zhaopin')
        self.cur.execute('USE Zhaopin')
        self.cur.execute('CREATE TABLE IF NOT EXISTS jobszhi('
                         'job_name varchar(200), '
                         'feedback_rate varchar(100), '
    ...
    def html_crawl(self, url_crawl):
        ...
        """
        try:
            response = request.Request(url_crawl, headers=self.headers)
    ...
    def job_info_store(self, job_info):
        ...
        for elem in job_info:
            self.cur.execute("INSERT INTO jobszhi (job_name, feedback_rate, company_name, salary, location)"
                             " values('{}', '{}', '{}', '{}', '{}')"
